---
title: "ISA 706 — Emphasis of Matter Paragraphs and Other Matter Paragraphs in the Independent Auditor's Report"
type: entity
created: 2026-04-07
updated: 2026-04-07
tags: [standards, isa, iaasb, reporting, emphasis-of-matter]
ref: "ISA 706"
status: active
---

# ISA 706 — Emphasis of Matter Paragraphs and Other Matter Paragraphs in the Independent Auditor's Report

## Objective

The objective of the auditor, having formed an opinion on the financial statements, is to draw users' attention, when in the auditor's judgment it is necessary to do so, by way of clear additional communication in the auditor's report, to:

- A matter, although appropriately presented or disclosed in the financial statements, that is of such importance that it is fundamental to users' understanding of the financial statements; or
- As appropriate, any other matter that is relevant to users' understanding of the audit, the auditor's responsibilities, or the auditor's report.

## Scope

ISA 706 (Revised) deals with additional communication in the auditor's report beyond what is required by other ISAs. It addresses Emphasis of Matter paragraphs (drawing attention to matters in the financial statements) and Other Matter paragraphs (drawing attention to matters not in the financial statements but relevant to understanding the audit or report).

When the auditor includes a Key Audit Matters section per [[1-standards/isa-701]], this ISA addresses the relationship between key audit matters and these additional paragraphs. Several other ISAs contain specific requirements for Emphasis of Matter or Other Matter paragraphs (listed in Appendices 1 and 2 of the standard).

## Key Requirements

### Emphasis of Matter Paragraphs (paras 8-9)

- **When to include:** If the auditor considers it necessary to draw users' attention to a matter presented or disclosed in the financial statements that is fundamental to users' understanding, provided:
  - The matter would not require a modified opinion under [[1-standards/isa-705]]; and
  - When [[1-standards/isa-701]] applies, the matter has not been determined to be a key audit matter.
- **Form requirements:**
  - Include in a separate section with a heading that includes the term "Emphasis of Matter"
  - Include a clear reference to the matter being emphasised and to relevant disclosures in the financial statements
  - Refer only to information presented or disclosed in the financial statements
  - Indicate that the auditor's opinion is not modified in respect of the matter emphasised

### Other Matter Paragraphs (paras 10-11)

- **When to include:** If the auditor considers it necessary to communicate a matter other than those presented or disclosed in the financial statements that is relevant to users' understanding of the audit, the auditor's responsibilities, or the auditor's report, provided:
  - Not prohibited by law or regulation; and
  - When [[1-standards/isa-701]] applies, the matter has not been determined to be a key audit matter.
- **Form requirements:** Include in a separate section with the heading "Other Matter" or other appropriate heading.

### Communication with Those Charged with Governance (para 12)

- If the auditor expects to include an Emphasis of Matter or Other Matter paragraph, the auditor shall communicate this expectation and the proposed wording to those charged with governance.

### Relationship with Key Audit Matters

- When ISA 701 applies, Emphasis of Matter paragraphs are not a substitute for describing individual key audit matters.
- Matters determined to be key audit matters may also be fundamental to users' understanding; in such cases, the auditor may highlight them more prominently within the Key Audit Matters section rather than using a separate Emphasis of Matter paragraph.
- A matter that is not a key audit matter (because it did not require significant auditor attention) may still warrant an Emphasis of Matter paragraph if fundamental to users' understanding (e.g., a subsequent event).

## Key Definitions

- **Emphasis of Matter paragraph:** A paragraph included in the auditor's report that refers to a matter appropriately presented or disclosed in the financial statements that, in the auditor's judgment, is of such importance that it is fundamental to users' understanding of the financial statements.
- **Other Matter paragraph:** A paragraph included in the auditor's report that refers to a matter other than those presented or disclosed in the financial statements that, in the auditor's judgment, is relevant to users' understanding of the audit, the auditor's responsibilities, or the auditor's report.

## Application to Probe Working Papers

No specific Probe working papers are currently mapped to ISA 706. The standard's requirements would be applied through [[2-working-papers/finalisation/02-50]] (Consideration of Assurance Report) when additional paragraphs are needed in the auditor's report.

## Related Standards

- [[1-standards/isa-700]] — Forming an Opinion and Reporting (base reporting requirements)
- [[1-standards/isa-701]] — Key Audit Matters (relationship between KAM and EOM/OM paragraphs)
- [[1-standards/isa-705]] — Modifications to the Opinion (EOM not used when modification required)
- [[1-standards/isa-800]] — Special Purpose Frameworks (requires EOM paragraph alerting readers)
- [[1-standards/isa-810]] — Summary Financial Statements
- [[concepts/isa-standards]] — Overview of ISA standards mapped to working papers

## References

- IAASB 2025 Handbook, Volume 1, pages 667-679
- Source: `raw/iaasb-handbook/IAASB-2025-Handbook-Volume-1.pdf`
